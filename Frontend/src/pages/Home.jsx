      ...
      Object.keys(tree[dept][year]).forEach(section => {
        Object.keys(tree[dept][year][section]).forEach(subject => {
          tree[dept][year][section][subject].forEach(note => {
            allNotes.push({ ...note, dept, year, section, subject });
          });
        });
      });
    });
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {recentNotes.map(note => (
              <Link
                key={note.id}
                to={`/${note.dept}/${note.year}/${note.section}/${note.subject}/${note.id}`}
                className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group"
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="p-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg group-hover:bg-blue-600 group-hover:text-white transition-colors text-blue-600">
                    <FileIcon />
                  </div>
                  <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-900 text-gray-500">{note.dept}</span>
                </div>
                <h4 className="font-bold text-sm mb-1 group-hover:text-blue-600 transition-colors line-clamp-1">{note.meta.title}</h4>
                <p className="text-[10px] text-gray-400 font-medium uppercase">{note.subject} • {note.year}</p>
              </Link>
            ))}
          </div>
        </div>
